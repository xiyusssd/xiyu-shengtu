...
export function ProviderEditor({
  ...
  async function save() {
    ...
    try {
      ...
      if (isOpenaiLike) {
        body.endpoint = draft.endpoint || undefined;
  ...
  async function test() {
    ...
    try {
      ...
      if (json.ok) {
        toast.success(
          `连通 · ${json.latencyMs ?? "?"}ms`,
          json.message ? { description: json.message } : undefined
        );
      } else {
        toast.error("连通失败", { description: json.message });
      }
    } catch (err) {
  ...
  return (
    <div className="flex h-full flex-col overflow-y-auto">
      ...
      {/* 内容区 */}
      <div className="flex-1 flex-col gap-6 px-6 py-6 flex">
        {/* 基本信息 */}
